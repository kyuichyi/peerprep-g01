...
const COLLAB_SERVICE_URL =
  process.env.COLLAB_SERVICE_URL || "http://collab-app:3004";
...
async function requestMatch(req, res) {
  ...
  const poll = async () => {
    ...
    if (Date.now() > deadline) {
      ...
      return res
        .status(408)
        .json({ status: "timeout", message: "No match found" });
    }
...
async function checkMatchStatus(req, res) {
  ...
  if (!status) {
    return res
      .status(404)
      .json({ error: "No match request found for this user" });
  }
...
async function questionMetaData(userAId, userBId, topic, difficulty) {
  ...
  const questionMetaData = await axios.post(
    `${QUESTION_SERVICE_URL}/api/questions/select`,
    {
      topicId: topic,
      difficulty,
      exclude: [...excludeSet],
